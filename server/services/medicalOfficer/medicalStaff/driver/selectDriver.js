...
async function getDrivers(clusterCenterNo){
    ...
    const rows = await db.query( `SELECT * FROM user INNER JOIN medical_staff ON user.userNic = medical_staff.userNic WHERE medical_staff.bloodCenterNo=? && medical_staff.medicalType = 'Driver'&& user.workStatus='1' `,[clusterCenterNo]);
    const data = rows;
    return  rows ;
