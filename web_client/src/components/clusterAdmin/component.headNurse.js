...
export default function HeadNurse() {
    ...
    function getHeadNurseData() {
        ...
        axios
            .post("http://localhost:8070/medicalstaff/selectHeadNurse", clusterAdmin)
            .then((res) => {
                setData(res.data.headNurses);
                console.log(res.data.headNurses);
                setLastRow(10);
                setLoading(!loading);
            })
            .catch((err) => {
    ...
    function addHeadNurse(e) {
        ...
        if (firstName.length == 0) {
            ...
            axios.post("http://localhost:8070/medicalstaff/headNurse", headNurse).then(
                (res) => {
                    //check password and username  
                    if (res['data']['message'] == "success") {
                        setWait(false);
                        setSuccessMessage("Head Nurse Added Sucessfully")
                        setSuccess(true);


                    } else {
                        setWait(false);
                        setUnsuccessMessage("Enter NIC Number already Registered");
                        setUnSuccess(true);
                    }
    ...
    function showUpdateHeadNurse(id, firstName, lastName, NIC, address, mobileNumber, email) {
    ...
    function removeHeadNurse(){
        setRemoveAlert(false);
        setWait(true);
        const headNurse = {headNurseNic};
        axios.post("http://localhost:8070/medicalstaff/removeHeadNurse", headNurse).then(
            (res) => {
                //check password and username  
                if (res['data']['message'] == "success") {
                    setWait(false);
    ...
    function updateHeadNurse(e) {
        ...
        if (updateFirstName.length == 0) {
            ...
            e.target.reset();
            // pass check the data with server 
            axios.post("http://localhost:8070/medicalstaff/updateHeadNurse", headNurse).then(
                (res) => {
                    //check password and username  
                    if (res['data']['message'] == "success") {
                        setWait(false);
        ...
        for (let i = firstRow; i < data.length; i++) {
            list.push(
                <> <tr>
                    ...
                    <td>{data[i]['email']}</td>
                    <td>
                        <button id='view-button-doctor-view' onClick={() => { showUpdateHeadNurse(data[i]['staffId'], data[i]['firstName'], data[i]['lastName'], data[i]['userNic'], data[i]['address'], data[i]['phoneNumber'], data[i]['email']) }}>View</button>
    ...
    function sucessbutton() {
        window.location = "/medicalstaff/headnurse";
    }
        ...
        <div>
            <div id="headNurse-contanier">
                <div id={`${success ? 'sucess-message-active' : 'sucess-message'}`}>
                    ...
                    <p id='sucess-message-box-headNurse-clusterAdmin'>{successMessage}</p> <br></br>
                    <button id="okay-button-headNurse-clusterAdmin" onClick={() => { setSuccess(sucessbutton) }}> Okay </button>
                </div>
                ...
                <div id={`${removeAlert ? 'remove-alert-clusteradmin-active' : 'sucess-message'}`}>
                    ...
                    <button id="remove-button" onClick={() => { removeHeadNurse() }}> Okay </button>
            ...
            <div
                id={`${formReg
                    ? "register-form-doctor-clusteradmin-active"
                    : "register-form-doctor-clusteradmin"
                    }`}
            >
                <h3 id="register-form-doctor-name-clusteradmin">Add New Doctor </h3>
                <form id="register-form-doctor-form-clusteradmin" onSubmit={addHeadNurse}>
                    {message ? (
            ...
            <div id={`${updateFormShow ? 'register-form-doctor-clusteradmin-active' : 'register-form-doctor-clusteradmin'}`}>
                ...
                <h3 id='register-form-doctor-name-clusteradmin'>Update HeadNurse </h3>
                <form id='register-form-doctor-form-clusteradmin' onSubmit={updateHeadNurse}>
